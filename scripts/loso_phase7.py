#!/usr/bin/env python3
"""
scripts/loso_phase7.py
=======================
4-fold LOSO for Phase 7 retraining.

Changes from Phase 6 (loso_phase2b_full.py):
  1. CLF training data: gold+aug+synthetic_phase7 (v1 440 + v2 314 = 754 synthetic examples)
     Synthetic covers CLF failures in priorities 5,6,7,8,10,11,15 from MODEL_FINDINGS.md.
     adr_spans are empty so synthetic is CLF-only — NOT passed to NER.
  2. Base model: DAPT (models/pubmedbert-ghana-dapt) for all fresh training.
     CLF is now always trained fresh (no seeding from stale Phase 6 checkpoints).
  3. NER training: unchanged (gold+aug+silver+newsletter_silver+demo_aug).
     NER warm-start seed: Phase 6 NER checkpoints (models_phase2b_qualitative_interview).
  4. Output dirs: models_phase7_<source>, fold_phase7_<source>.json.

Deferred synthetic priorities: 1 (FDA/regulatory), 4 (Akan/Ewe/Hausa) — need real records.

Usage (from ghana-adr-pipeline/):
    python scripts/loso_phase7.py
    python scripts/loso_phase7.py --source cohort_study
    python scripts/loso_phase7.py --skip-warmstart
"""
...
SYNTHETIC_CLF_V1  = ROOT / "data" / "silver" / "synthetic_phase7_v1.jsonl"
SYNTHETIC_CLF_V2  = ROOT / "data" / "silver" / "synthetic_phase7_v2.jsonl"
REPORTS   = ROOT / "reports" / "loso"
REPORTS.mkdir(parents=True, exist_ok=True)

# Phase 7 uses the DAPT model as base for all fresh training
MODEL_NAME = str(ROOT / "models" / "pubmedbert-ghana-dapt")

# NER warm-start: copy Phase 6 NER best for qualitative_interview fold
# Created automatically in main() from models_phase2b_qualitative_interview/ner_best
NER_WARMSTART_DIRS = {
    "qualitative_interview": REPORTS / "models_phase7_qualitative_interview_ner_seed",
}
...
# Phase 6 macro scores as baseline for delta column
PHASE6_BASELINE = {
...
log.info("[Phase 7 LOSO] importing training modules...")
...
def load_silver_ner():
    """DailyMed insert silver — NER only."""
    rows = []
    with open(SILVER, encoding="utf-8") as f:
        for line in f:
            if not line.strip(): continue
            r    = json.loads(line)
            sid  = "silver_" + hashlib.md5(r["sentence"].encode()).hexdigest()[:10]
            text = r["sentence"]
            entities = []
...
def load_newsletter_silver():
    if not NEWSLETTER_SILVER.exists():
        log.warning(f"  [newsletter silver] not found: {NEWSLETTER_SILVER.name} -- skipping")
        return []
    rows = []
    with open(NEWSLETTER_SILVER, encoding="utf-8") as f:
        for line in f:
            if not line.strip(): continue
            r    = json.loads(line)
            sid  = "nl_" + hashlib.md5(r["sentence"].encode()).hexdigest()[:10]
            text = r["sentence"]
            entities = []
            for span in r.get("adr_spans", []):
                entities.append({"label": "ADR",
...
def load_synthetic_clf():
    """Synthetic Phase 7 CLF examples — CLF training only, NOT passed to NER."""
    rows = []
    for path in [SYNTHETIC_CLF_V1, SYNTHETIC_CLF_V2]:
        if not path.exists():
            log.warning(f"  [synthetic clf] not found: {path.name} -- skipping")
            continue
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                rows.append({
                    "id":          r["setid"],
                    "text":        r["sentence"],
                    "label":       r["contains_adr"],
                    "source_type": "synthetic_phase7",
                    "synthetic":   True,
                })
    return rows
...
# ═══════════════════════════════════════════════════════════════════════════════
# Evaluation — Pass A (overrides ON, using inference_engine_v2 rules)
# ═══════════════════════════════════════════════════════════════════════════════
...
def run_fold(source, gold_clf, gold_ner, aug_clf, aug_ner,
             silver_ner, tokenizer, device, synthetic_clf=None,
             demo_aug_ner=None, newsletter_silver_ner=None,
             skip_warmstart=False, interview_adr_boost=1.0):

    fold_dir  = REPORTS / f"models_phase7_{source}"
    fold_json = REPORTS / f"fold_phase7_{source}.json"
    ...
    # CLF: gold+aug+synthetic (Phase 7 change — synthetic targets model-level failures)
    clf_subtrain, clf_val = make_val_split(
        clf_train_gold, lambda r: (r["label"], r.get("source_type", "")))
    clf_subtrain = clf_subtrain + clf_train_aug + _synthetic

    # NER: gold+aug+silver+demo_aug+newsletter_silver — unchanged from Phase 6
    ...
    n_syn_adr  = sum(1 for r in _synthetic if r["label"])
    n_syn_non  = len(_synthetic) - n_syn_adr

    log.info(f"\n{'='*64}")
    log.info(f"  FOLD  hold-out={source}  [Phase 7: CLF=gold+aug+synthetic, NER=gold+aug+silver]")
    log.info(f"  clf  test={n_total} ({n_positive} pos, {pct_pos}%)"
             f"  subtrain={len(clf_subtrain)}"
             f" (gold={len(clf_train_gold)}, aug={len(clf_train_aug)}"
             f", synthetic={len(_synthetic)} [{n_syn_adr}A/{n_syn_non}N])  val={len(clf_val)}")
    ...
    # CLF: always train fresh (training data changed — cannot reuse Phase 6 checkpoints)
    ...
    if clf_ckpt.exists() and any(clf_ckpt.iterdir()):
        ...
        log.info(f"  -> Training CLF fresh from DAPT (adr_boost={clf_adr_boost})...")
        t0 = time.time()
    ...
    # NER: warm-start from Phase 6 NER for qualitative_interview; fresh for others
    ...
    if ner_ckpt.exists() and any(ner_ckpt.iterdir()):
        ...
        log.info(f"  -> NER warm-start from Phase 6 ({NER_WARMSTART_EPOCHS} epochs)...")
        t0 = time.time()
        ...
        log.info("  -> Training NER fresh from DAPT...")
        t0 = time.time()
        ner_ckpt = train_ner_fold(fold_dir, ner_subtrain, ner_val, tokenizer, device)
    ...
    base  = PHASE6_BASELINE.get(source, {})
    ...
    log.info(f"  CLF  P={clf_m['clf_precision']:.4f}  R={clf_m['clf_recall']:.4f}"
             f"  F1={clf_m['clf_f1']:.4f}  (vs Phase6: {clf_d:+.4f})")
    log.info(f"  NER  overall F1={ner_m['ner_overall_f1']:.4f}  (vs Phase6: {ner_d:+.4f})")
    for tag in ENTITY_TAGS:
    ...
    result = {
        ...
        "silver_mode":        "Phase7: CLF=gold+aug+synthetic NER=gold+aug+silver",
        "n_synthetic_clf":    len(_synthetic),
        "n_total":            n_total,
        ...
        "phase6_clf_f1":      base.get("clf_f1"),
        "phase6_ner_f1":      base.get("ner_overall_f1"),
        "clf_delta_vs_phase6": clf_d,
        "ner_delta_vs_phase6": ner_d,
        **clf_m,
        **ner_m,
    }
...
def _macro(results):
    ...
    avg["clf_delta_vs_phase6"] = round(
        sum(r.get("clf_delta_vs_phase6", 0) for r in results) / len(results), 4)
    avg["ner_delta_vs_phase6"] = round(
        sum(r.get("ner_delta_vs_phase6", 0) for r in results) / len(results), 4)
    avg["ner_per_tag"] = {}
    for tag in ENTITY_TAGS:
...
def write_summary(results):
    path  = REPORTS / "summary_phase7.md"
    macro = _macro(results)

    n_syn = results[0].get("n_synthetic_clf", 0) if results else 0
    header = (
        "# LOSO Summary — Phase 7 (Pass A, overrides ON)\n\n"
        f"Model: CLF = gold+aug+synthetic_phase7 ({n_syn} synthetic) | "
        "NER = gold+aug+silver (DailyMed+newsletter)\n"
    ...
    def fmt_row(label, n, pct, r):
        ...
        clf_d = r.get("clf_delta_vs_phase6", 0)
        ner_d = r.get("ner_delta_vs_phase6", 0)
        return (
            f"| {label:<22}"
            f"| {str(n):<4}"
            f"| {str(pct):<5}"
...
def main():
    parser = argparse.ArgumentParser(description="4-fold LOSO — Phase 7")
    ...
    parser.add_argument("--skip-warmstart", action="store_true",
                        help="Skip NER warm-start from Phase 6 checkpoint")
    parser.add_argument("--interview-adr-boost", type=float, default=1.0)
    args = parser.parse_args()

    log.info(f"\n{'='*64}")
    log.info("  Ghana ADR -- Phase 7 LOSO (4 folds, Pass A)")
    log.info(f"  CLF: gold+aug+synthetic_phase7 | NER: gold+aug+silver")
    log.info(f"  Base model: DAPT ({MODEL_NAME})")
    ...
    n_syn_adr = sum(1 for r in synthetic_clf if r["label"])
    n_syn_non = len(synthetic_clf) - n_syn_adr
    log.info(f"  Gold: {len(gold_clf)} clf  {len(gold_ner)} ner"
             ...
             f"  Synthetic CLF: {len(synthetic_clf)} ({n_syn_adr}A/{n_syn_non}N)")

    # Create frozen NER warmstart seed from Phase 6 NER best.
    for ws_source, ws_dir in NER_WARMSTART_DIRS.items():
        ...
        if src_ner.exists() and not seed_ner.exists():
            log.info(f"  Creating Phase 7 NER warmstart seed for {ws_source}")
            ws_dir.mkdir(parents=True, exist_ok=True)
            shutil.copytree(src_ner, seed_ner)
        elif not src_ner.exists():
            log.warning(f"  [warmstart] Phase 6 NER checkpoint missing for {ws_source} -- warm-start disabled")
    ...
    if len(all_results) == len(sources):
        ...
        log.info(f"\n{'='*64}")
        log.info("  RESULTS — Phase 7 LOSO")
        log.info(f"{'='*64}")
        for r in all_results:
            sev = r["ner_per_tag"]["SEVERITY"]
            log.info(
                f"  {r['held_out_source']:<28}"
                f"  CLF_F1={r['clf_f1']:.3f} ({r['clf_delta_vs_phase6']:+.3f} vs Ph6)"
                f"  NER_F1={r['ner_overall_f1']:.3f} ({r['ner_delta_vs_phase6']:+.3f} vs Ph6)"
    ...
    log.info("  Phase 7 LOSO complete.")
